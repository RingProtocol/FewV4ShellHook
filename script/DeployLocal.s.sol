...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        // 5. Mint underlying to PoolManager for flash-take during lp route
        ...
        // Derive lp key same way as hook
        ...
        manager.initialize(lpKey, lpInitPrice);
        console2.log("Lp pool initialized at price:", lpInitPrice);
        ...
        IFewWrappedToken(lpToken0).wrap(LIQUIDITY);
        IFewWrappedToken(lpToken1).wrap(LIQUIDITY);
        ...
        IFewWrappedToken(lpToken0).wrap(wrapAmount);
        IFewWrappedToken(lpToken1).wrap(wrapAmount);
        IERC20(lpToken0).transfer(address(manager), wrapAmount);
        IERC20(lpToken1).transfer(address(manager), wrapAmount);
        ...
        // 12. Test swap: route zeroForOne through the lp pool
